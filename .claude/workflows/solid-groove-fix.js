...
This is a **bug fix**, not a feature. Your checks on published contracts, the domain invariants, resource lifecycle, scope and PR size apply unchanged. Check 2a — the frozen acceptance contract — narrows to one thing: \`git diff origin/${BASE_BRANCH}..${fix.branch} -- docs/core-flows.md docs/prd.md tests/e2e/emulator/flows\` should be **empty**. A bug fix does not edit the specification and does not remove a \`test.fixme\` marker; either is blocking.
