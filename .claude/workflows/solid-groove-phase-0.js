...
// every agent runs on Opus. Contract-owning tasks once justified Opus alone,
// because an error there propagates into every dependent task and surfaces late,
// but code-writing is now Opus across the board. Review is always Opus, at high
// effort, and runs before any PR is opened.
...
// One task: implement (Opus) -> review (Opus) -> fix ->
...
async function runTask(t) {
  ...
  const pr = await agent(prPrompt(t, impl), { model: 'opus', label: `pr:${t.id}`, phase: t.phase })
