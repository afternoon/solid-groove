...
// The GitHub issues, not this file, are the source of truth for what is done. A
// closed issue means the task landed; an open one is still outstanding. The
// script body has no `gh` and no MCP, so a single bootstrap agent reads every
// task's issue once at the start of the run and returns its state. The scheduler
// then seeds "already landed" from the closed issues instead of assuming a task
// is complete only if THIS run implemented it — which is what makes a resume
// correct: closed tasks are skipped and count as satisfied dependencies, and
// only genuinely-open tasks are implemented. `hasOpenPr`/`openPrNumber` surface a
// branch a prior interrupted run already pushed, so the scheduler can flag it
// rather than silently open a duplicate.
const STATUS_SCHEMA = {
  type: 'object',
  required: ['statuses'],
  properties: {
    statuses: {
      ...
      items: {
        type: 'object',
        required: ['id', 'issue', 'state'],
        properties: {
          id: { type: 'string', description: 'Task id, e.g. LOOP-004' },
          issue: { type: 'number', description: 'GitHub issue number read' },
          state: { type: 'string', enum: ['OPEN', 'CLOSED'], description: 'Issue state, upper-cased' },
          blockedLabel: { type: 'boolean', description: 'True if the issue carries the `blocked` label' },
          ...
          openPrBranch: { type: 'string', description: 'The open PR head branch, if any' },
        },
      },
    },
  },
...
// Read the live state of every Alpha Milestone 1 issue in one agent so the scheduler can
// treat the issues as the source of truth. Whoever runs this brief has the
// `mcp__github__*` tools; the script body does not.
const statusPrompt = (tasks) => `You are reading GitHub issue state for the Solid Groove Alpha Milestone 1 workflow, in \`afternoon/solid-groove\`. Use the \`mcp__github__*\` tools; there is no \`gh\` CLI. Do not modify anything — this is read-only.

For EACH of the following task/issue pairs, read the issue and report its current state exactly as GitHub returns it. Do not infer state from the backlog file, from comments, or from what you expect — read the issue itself.

${tasks.map((t) => `- ${t.id} -> issue #${t.issue}`).join('\n')}

For each, report:
- \`state\`: "CLOSED" if the issue is closed (the task has landed), otherwise "OPEN". Upper-case.
- \`blockedLabel\`: true if the issue carries a label named \`blocked\`.
- \`hasOpenPr\`/\`openPrNumber\`/\`openPrBranch\`: if an OPEN pull request already exists that closes or references this issue (search PRs for \`Closes #<issue>\` or a branch named for the task id), report it. This flags work a prior interrupted run left behind. Omit if none.

Return one entry per pair, using the exact task ids given.`
...
// Alpha Milestone 1 is scheduled from two live inputs, neither hand-maintained: the
// GitHub issues decide what is already done (a closed issue = a landed task), and
// the `deps` graph decides the order among what remains. The backlog calls its
// Dependencies lines "the machine-readable work graph", and hand-maintained waves
// drift from it silently: a dependency added to the backlog but not to the wave
// list produces a task that runs before what it needs, and nothing detects it.
// This loop cannot drift — it reads the issue state and derives the order every run.
//
// Each round takes every open task whose Alpha Milestone 1 dependencies have all landed —
// where "landed" means its issue is closed OR it was approved earlier this run —
// and runs them concurrently. The concurrency cap queues the excess, so a wide
// round is safe. Alpha Milestone 0 dependencies are deliberately absent from `deps`: they
// are already on the base branch, and a scheduler can only satisfy dependencies
// it runs.
const results = []
const landed = new Set()

// Read the live issue state before scheduling anything. The issues are the
// source of truth: a closed issue is a landed task, so it is seeded into
// `landed` (it satisfies dependents and is never re-run) and dropped from the
// work list. An open issue is outstanding and gets scheduled once its
// dependencies land. This is what makes `Workflow({ name: 'solid-groove-phase-1' })`
// a correct resume on its own — no subset argument needed to skip finished work.
phase(TASKS[0].phase)
log('Reading GitHub issue state to decide what is complete, ready, and blocked')
const statusReport = await agent(statusPrompt(TASKS), {
  model: 'sonnet',
  label: 'status:alpha-m1',
  phase: TASKS[0].phase,
  schema: STATUS_SCHEMA,
})

// A failed status read must stop the run, not silently fall back to "everything
// is open" — that would re-implement finished tasks and open duplicate PRs,
// exactly what reading the issues is meant to prevent.
if (!statusReport || !Array.isArray(statusReport.statuses)) {
  throw new Error('Could not read issue state from GitHub; refusing to schedule without the source of truth.')
}
const statusById = new Map(statusReport.statuses.map((s) => [normaliseId(s.id), s]))
const missingStatus = TASKS.filter((t) => wanted(t.id) && !statusById.has(normaliseId(t.id)))
if (missingStatus.length) {
  throw new Error(`Issue state missing for: ${missingStatus.map((t) => t.id).join(', ')}. Refusing to schedule blind.`)
}

// Seed landed from every closed issue, whether or not the caller asked to run it.
// A closed dependency of a requested task must count as satisfied even when it is
// not itself in this run.
for (const t of TASKS) {
  if (statusById.get(normaliseId(t.id))?.state === 'CLOSED') landed.add(t.id)
}

const done = TASKS.filter((t) => landed.has(t.id))
if (done.length) log(`Already complete (issue closed), skipping: ${done.map((t) => t.id).join(', ')}`)
for (const t of TASKS) {
  const st = statusById.get(normaliseId(t.id))
  if (wanted(t.id) && !landed.has(t.id) && st?.hasOpenPr) {
    log(`${t.id}: an open PR already exists (#${st.openPrNumber ?? '?'} ${st.openPrBranch ?? ''}) from a prior run — it will be re-implemented on a fresh claude/${t.id.toLowerCase()} branch; reconcile the stale PR by hand.`)
  }
}

// Reconcile the issue's `blocked` label against this file's decision-gate list
// (`t.blocked`, which drives the decisionBlocked brief). They should agree; when
// they drift, the issue is authoritative for scheduling but the brief still needs
// the specific DEC ids, so surface the mismatch rather than resolve it silently.
for (const t of TASKS) {
  if (!wanted(t.id) || landed.has(t.id)) continue
  const labelled = statusById.get(normaliseId(t.id))?.blockedLabel === true
  const gated = Boolean(t.blocked)
  if (labelled && !gated) {
    log(`${t.id}: issue carries the \`blocked\` label but this workflow lists no decision gate for it — the brief will not name a decision; check the issue before merging.`)
  } else if (!labelled && gated) {
    log(`${t.id}: this workflow gates it on ${t.blocked.join(', ')} but the issue no longer carries the \`blocked\` label — a decision may have been made; the brief still holds the agent back.`)
  }
}

// The work list is the requested tasks that are still open. Closed tasks are
// already recorded in `landed`, so they neither run nor block their dependents.
const pending = TASKS.filter((t) => wanted(t.id) && !landed.has(t.id))
if (!pending.length) {
  log('Nothing to do — every requested Alpha Milestone 1 task already has a closed issue.')
  return { results: [], approved: [], notApproved: [], alreadyComplete: done.map((t) => t.id) }
}

// A dependency is satisfied when it landed (closed issue or approved this run) or
// when the caller filtered it out — running `LOOP-010` alone must not deadlock on
// a `LOOP-005` this invocation was never asked to run. Both are sound because a
// filtered-out or closed dependency is already merged (or knowingly excluded).
...
return {
  results,
  alreadyComplete: done.map((t) => t.id),
