...
│   ├── Transport.ts         # Play/pause/stop/seek, playhead, tempo and loop mirrors, metronome
...
- `Transport.ts` owns the session-scoped playhead: play/pause/stop/seek/continue, the 40-240 BPM clamp, and the metronome. It is written against an injectable `TransportEngine` rather than `Tone.getTransport()`, and it never schedules the arrangement — a tempo change is mirrored onto the transport, not applied by rebuilding nodes. Tempo lives in the song and is written only by a `parameter.set` command; this layer mirrors it. So does the loop: `song.loop` (range plus toggle) is project state written only by `loop.setRange`/`loop.setEnabled`, and `applyLoop` mirrors it onto the engine in place, so widening the brace mid-playback is honoured from the transport's next pass without restarting the song. Bar alignment is `domain/time.ts`'s `barAlignedRange`, next to the invariant that enforces it, so a stored range and an engine-side one cannot disagree about where a bar line is. `underrun.ts` counts late dispatches and reports them *sampled* as `audio_underrun`; it compares an event's intended time against `ProjectAudioGraph.audioClockNow()` (the context's true `currentTime`), never `Tone.now()`, which is `currentTime + lookAhead` and would score healthy playback as a continuous stream of drops.
