...
Content is **synthesized by this repository**, not downloaded. `DEC-003` has not approved any content source, and section 3 forbids bundling third-party audio without per-asset raw-redistribution rights and archived evidence; owner-created content is the route section 14 item 6 already prescribes, and it needs no decision. An agent must not satisfy this task by pulling files from a sample site, a search API, or a model.
