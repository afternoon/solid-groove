...
2. Keep the change vertical and self-contained: product code, tests, fixtures, and documentation for that task in one PR. The PR body links its issue and states the evidence. **Any change that alters the UI includes a screenshot** (before/after, or a short GIF for an interaction) in the PR body, naming the route/view and the theme — this is how reviewers see the visual result without checking out the branch. A PR with no user-visible change says so instead. The repository PR template (`.github/pull_request_template.md`) has the section.
