...
- Which AI provider, model budget, usage limit, and data-retention policy are acceptable for the alpha? The transcript retention window is settled at 30 days (AI-09); what remains open is the wording of the disclosure that states it, and whether the chosen provider may retain or train on requests — which the same disclosure has to state alongside our own retention. (`DEC-005`.)
