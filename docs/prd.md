...
- **No project content, ever.** Project, track, clip, section, and asset *names*; note and automation data; assistant prompts and replies; user-entered search terms; file names; asset URLs; email addresses; and authentication tokens are never event parameters. Where an entity must be identified, the prefixed ID (section 9.4) is logged, not its name. This rule is unchanged by the assistant-conversation store below: that store is first-party and is never a source of event parameters or of anything sent to a third-party processor.
...
- Session Replay is enabled to understand how the product is used, not to access the user's music ([ADR 0002](./adr/0002-sentry-session-replay.md), superseding [ADR 0001](./adr/0001-sentry-for-error-monitoring.md) decision 4; canvas capture revised by [ADR 0003](./adr/0003-replay-canvas-capture-and-assistant-conversation-retention.md)). Text is masked and media blocked at capture, so clip and track names, typed text, and the assistant conversation are never serialized into the payload. **Canvas capture is on**: the arrangement, piano roll, and waveform displays are recorded, because the interactions worth observing — dragging a clip edge, placing a note, navigating the arrangement — happen there and a replay with them blanked cannot show them. Sampled recordings therefore include the user's musical structure, which is a deliberate expansion accepted for that purpose and limited by it. Session sampling is low and error-triggered replay stays off. The content rule above applies to the replay payload and is tested against it. The disclosure names replay, says recordings may include the arrangement, and declining telemetry stops replay capture at the source rather than filtering it before send.
- Assistant conversations are retained as **first-party** data for product analysis — understanding what people ask the assistant and which replies they accept — under [ADR 0003](./adr/0003-replay-canvas-capture-and-assistant-conversation-retention.md). They are stored in our own Firestore under the project that produced them, and are never sent to Sentry, never unmasked in replay, and never event parameters; the content rules above hold verbatim and their tests are unchanged. Retention carries its own gates: conversations are owner-scoped by security rule, deleting a project deletes its conversations, an account deletion request removes them, retention is bounded (`DEC-009` sets the bound), and declining telemetry excludes a user's conversations from retention-for-analysis without costing any assistant capability. The disclosure states specifically that assistant conversations are stored and why.
...
> **Deferred to post-alpha (`DEC-011`).** The four events above still fire and are still verified by automated tests — the instrumentation ships with its features as usual. What is deferred is *computing and acting on this measure*: the threshold (three sections, two minutes), the definition of an "activated" project, and whether track progression is the right primary measure at all need more consideration than the alpha can give them, and a measure adopted casually is harder to withdraw than one adopted late. `OPS-001` therefore does **not** verify that this measure computes from real cohort data. See section 16.
