...
- Retention is **30 days** from when a message was sent, after which the transcript is deleted rather than archived or aggregated into something that outlives it. The window is a decided product commitment (`DEC-005`): it is stated in the disclosure, enforced as configuration in one place, and lengthened only by another product decision that updates the disclosure with it. The disclosure wording itself is still owed by `DEC-005`.
- Our retained transcripts are used only to review and improve Solid Groove, and are not sold or shared beyond that purpose. Separately, the AI provider may retain the requests we send it and use them to train its models (`DEC-005`); that is a decided term of the alpha, not an oversight, and the disclosure states it in the user's words rather than burying it in a linked policy.
- The disclosure does not overstate what the opt-out controls. Declining stops Solid Groove retaining conversations and deletes what it has retained; it does not withdraw what has already been sent to the provider or reach into the provider's own retention. A user must not be able to read the control as a promise it cannot keep. Whether to offer a provider-level opt-out — declining the assistant entirely is already possible under AI-06 — is a post-alpha question to be reopened on cohort feedback, not a gap this requirement papers over.
...
- Opting out stops retention for later messages and deletes that user's already-retained transcripts rather than letting them run out the remainder of their 30 days.
...
- Users are told what project data is sent to an AI provider, and that the provider may retain and train on it, and can decline assistant use without losing manual DAW features. Declining the assistant is the alpha's only way to keep prompts from the provider, and the AI-09 disclosure says so rather than implying the transcript opt-out goes further than it does.
- Assistant transcripts are retained for 30 days (AI-09), only to review and improve the product, and only until the user opts out or that window expires. The opt-out is durable, account-scoped, enforced server-side, and costs no capability. Transcripts live in their own store: the analytics and error pipelines carry no conversation text whatever this setting says.
...
- An assistant transcript is retained for 30 days from the message that produced it, then deleted, and is read only to review and improve the product (AI-09). Retention is disclosed before the user's first assistant message and can be declined at any time from a durable, account-scoped opt-out that costs no capability. Transcripts are stored apart from the OPS-02/OPS-03 pipelines, which continue to carry no conversation text at all.
- The AI provider may retain the requests Solid Groove sends it and train its models on them (`DEC-005`). The alpha offers no provider-level opt-out: a user who does not want their prompts to reach the provider declines the assistant, which AI-06 already guarantees costs no manual DAW capability. The AI-09 disclosure states the provider's posture plainly and does not let the transcript opt-out imply control over it. Adding a provider-level opt-out is reopened if cohort feedback (`HARD-005`) shows it matters to the target user; `P2-005` holds that work.
...
- Which AI provider, model budget, usage limit, and acceptable off-platform project context apply to the alpha? The data-handling half of this decision is settled: transcripts are retained for 30 days, and the provider may retain and train on requests (AI-09). What remains is the provider choice and its commercial envelope, plus final approval of the disclosure copy an implementation agent drafts against those decided terms. (`DEC-005`.)
