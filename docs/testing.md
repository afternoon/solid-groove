...
**Two different `cpal` failures exist and must not be conflated.** The one above is `default_output_config: DeviceUnavailable`, thrown at *import* on a host with **no** output device, and fixed by the null ALSA device. A second, distinct one is `play_output_stream ... OSStatus 1937010544`, thrown mid-run on a host that **has** a real device which is *contended* by concurrent processes — many parallel worktrees each running `bun run test`, or anything else on the machine holding the default output. A null ALSA device does not fix that one, and its absence is not its cause; see [#203](https://github.com/afternoon/solid-groove/issues/203).

The fix for a suite hitting the second failure is to stop acquiring the output device when it never needed it. `src/audio/testAudioTeardown.test.ts` is the worked example: it only asserts that the guard wraps `dispatchEvent`, so it builds its node from an `OfflineAudioContext` rather than an `AudioContext` and touches no output stream at all. A third test there pins the substitution — it asserts the node still resolves `dispatchEvent` to the guard's own installed wrapper, so the file cannot quietly start testing the *unguarded* inherited implementation. Note `OfflineAudioContext` has no `close()`; there is no stream to release. **This only applies to a suite that does not render.** The four suites named above genuinely render audio and must keep a real context, so they remain exposed to contention — do not "fix" them this way, and never mock Tone, skip a file, or weaken `src/audio/testAudioContext.ts` to get a green run.
