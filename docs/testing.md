...
1. **Hosting**: `firebase hosting:versions:list --project "$FIREBASE_PROJECT_ID"` lists every past release. `firebase hosting:clone <project-id>:<PREVIOUS_VERSION_ID> <project-id>:live --project "$FIREBASE_PROJECT_ID"` republishes an earlier version as the live release immediately, without rebuilding.
2. **Firestore rules**: every past `firestore.rules` revision is already in git history against the commit SHA its `deploy` job run stamped. Roll back with `git show <previous-commit>:firestore.rules > /tmp/firestore.rules.rollback && firebase deploy --only firestore:rules --project "$FIREBASE_PROJECT_ID"` after pointing the deploy at that file (e.g. a short-lived branch, or `firebase.json`'s `firestore.rules` path temporarily repointed for the one deploy).
