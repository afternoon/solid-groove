...
import { captureAllEvidence, ingestAll, writeAcquiredBundle } from "./acquire/ingest.mjs";
import {
  LOCKFILE_PATH,
  readLockfile,
  selectionsBySource,
...
export const QUARANTINE_DIR = join(REPO_ROOT, "public", "samples", "quarantine");

/** Gitignored: prepared acquired masters, merged into the manifest by `library:build`. */
export const ACQUIRED_DIR = join(REPO_ROOT, "public", "samples", "acquired-library");
...
export function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    // bare `library:producer-space` does something useful).
      ...
      args.bulk = argv[i + 1]?.startsWith("--") ? "freepats" : (argv[++i] ?? "freepats");
...
function reportPlan(lockfile, log) {
  ...
  if (lockfile.selections.length === 0) {
    ...
    log("");
    log("To add content: choose specific files from an approved source, add them to");
    log(`${LOCKFILE_PATH}, run --pin to record their checksums, review, and commit.`);
    return;
  }
  ...
  if (drafts.length > 0) {
    ...
    log("");
    log("Run `bun run library:acquire -- --pin` to download and record their checksums.");
  }
}
...
export async function acquire({
  ...
  if (listBulk || bulk) {
    ...
    if (listBulk) {
      ...
      log("");
      log("Confirm the archive is CC0 on its page, set archiveUrl to the exact .zip,");
      log("then run e.g. `bun run library:producer-space -- <bulk-source-id>`.");
    ...
    for (const source of sources) {
      ...
      log(`evidence:  ${join(EVIDENCE_DIR, result.evidencePath.split("/").pop())}`);
  ...
  log("");
  log(`ingested ${ingested.length} assets from ${Object.keys(evidence).length} sources`);
