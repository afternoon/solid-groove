...
function fixtureWav({
	...
	return encodeWav(
		stereo ? [make(0), make(stereo === "wide" ? 1.2 : 0)] : make(0),
		rate,
	);
}
...
describe("lockfile validation", () => {
	...
	it("enforces the per-source selection ceiling", () => {
		...
		expect(
			validateLockfile({ version: 1, selections: many }).join("\n"),
		).toMatch(/over its \d+ ceiling/);
	});
...
describe("download", () => {
	...
	it("refuses plain http", async () => {
		await expect(download("http://example.com/kick.wav")).rejects.toThrow(
			/non-https/,
		);
	});
	...
	it("surfaces a failed request rather than writing an error page as audio", async () => {
		...
		await expect(
			download("https://example.com/gone.wav", { fetchImpl }),
		).rejects.toThrow(/404/);
	});
});
...
describe("quarantine", () => {
	...
	it("refuses to ingest anything unpinned unless pinning", async () => {
		...
		await expect(
			fetchToQuarantine(selection, { quarantineDir }),
		).rejects.toThrow(/no pinned sha256; run with --pin/);
		await expect(
...
describe("archives", () => {
	...
	it("recognizes archives and explains unsupported ones", () => {
		...
		expect(unsupportedArchiveReason("bank.tar.bz2")).toMatch(
			/bzip2[\s\S]*\.zip/,
		);
...
describe("audio preparation", () => {
	...
	it("reports a file it cannot decode instead of writing silence", async () => {
		await expect(
			decodeToSamples(Buffer.from("not audio at all")),
	...
	it("detects dual-mono and collapses it", () => {
		...
		expect(
			isEffectivelyMono([channel, new Float32Array([0.9, 0.1, -0.5])]),
		).toBe(false);
		expect(
			Array.from(
				downmixToMono([new Float32Array([1, 1]), new Float32Array([0, 0])]),
			),
		).toEqual([0.5, 0.5]);
	});
...
describe("end-to-end ingest", () => {
	...
	async function ingestFixture(overrides = {}, fixtureOptions = {}) {
		...
		const archive = Buffer.from(
			zipSync({ "pack/hit.wav": new Uint8Array(wav) }),
		);
	...
	it("fails on an archive member whose bytes changed", async () => {
		await expect(
			ingestFixture({ memberSha256: "d".repeat(64) }),
		).rejects.toThrow(/archive member checksum mismatch/);
	});

	it("assigns acquired ids from a range that cannot collide with generated ones", () => {
		expect(acquiredAssetId("drums", "kick", 0)).toBe(
			"sg-one-shot-drums-kick-5001",
		);
