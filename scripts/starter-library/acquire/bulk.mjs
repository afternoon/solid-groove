...
export async function acquireBulkSource(
	...
	if (!bytes) {
		...
		if (unsupported)
			throw new Error(`bulk source "${source.id}": ${unsupported}`);
	...
	const members = listArchiveMembers(bytes).filter((m) =>
		AUDIO_MEMBER.test(m.name),
	);
	const capped = source.maxMembers
		? members.slice(0, source.maxMembers)
		: members;
	...
	for (let i = 0; i < capped.length; i++) {
		...
		try {
			...
			log(
				`  skipped ${member.name}: ${error instanceof Error ? error.message : error}`,
			);
		}
	}
