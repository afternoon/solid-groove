...
import { extractMember, isArchive, unsupportedArchiveReason } from "./archive.mjs";
...
export function resolveSelectedBytes(selection, downloaded) {
  ...
  if (!isArchive(selection.originalFilename)) {
    const reason = unsupportedArchiveReason(selection.originalFilename);
    throw new Error(reason ?? `${selection.originalFilename} is not a supported archive`);
  }
...
export async function ingestSelection(selection, context) {
  ...
  if (!source) throw new Error(`${selection.id}: unknown source ${selection.sourceId}`);
  ...
  return {
    ...
    asset: {
      ...
      audio: {
        ...
        tuningCents: selection.asset.rootNote ? (selection.asset.tuningCents ?? 0) : null,
        bpm: null,
        bars: null,
        loopable: false,
        chokeGroup: selection.asset.choke ? `${selection.asset.family}-hat` : null,
...
export async function ingestAll(lockfile, context) {
  ...
  for (const selection of lockfile.selections) {
    ...
    const withId = {
      ...selection,
      assetId: acquiredAssetId(selection.asset.family, selection.asset.role, index),
    };
    try {
...
export async function captureAllEvidence(lockfile, { evidenceDir, fetchImpl, now }) {
  const used = [...new Set(lockfile.selections.map((selection) => selection.sourceId))];
  const captured = {};
