...
export function writeLockfile(lockfile, { path = LOCKFILE_PATH, updatedAt } = {}) {
  const sorted = {
    version: LOCKFILE_VERSION,
    updatedAt: updatedAt ?? lockfile.updatedAt ?? null,
    selections: [...lockfile.selections].sort((a, b) => a.id.localeCompare(b.id)),
  };
...
export function validateLockfile(lockfile) {
  ...
  for (const selection of lockfile.selections) {
    ...
    if (seenIds.has(selection?.id)) errors.push(`${where}: duplicate selection id`);
    ...
    if (!HEX_64.test(selection?.sha256 ?? "")) {
      errors.push(`${where}: sha256 must be the SHA-256 of the downloaded bytes`);
    }
    if (!(selection?.bytes > 0)) errors.push(`${where}: bytes must be positive`);
    if (selection?.archiveMember && !HEX_64.test(selection?.memberSha256 ?? "")) {
    ...
    if (!asset) {
      ...
      if (asset.sourceType === "synthesized") {
        errors.push(`${where}: acquired audio cannot claim sourceType "synthesized"`);
      }
...
export function validateDraft(selection) {
  ...
  if (!asset) {
    ...
    if (!asset.characters?.length) errors.push(`${where}: a character tag is required`);
    validatePackAssignment(asset, findSource(selection?.sourceId), where, errors);
  }
  return errors;
}
...
function validatePackAssignment(asset, source, where, errors) {
  ...
  if (!pack) {
    errors.push(`${where}: asset.pack "${asset.pack}" is not a registered pack`);
    return;
  }
