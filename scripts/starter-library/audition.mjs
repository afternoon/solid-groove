...
export async function audition({ port = DEFAULT_PORT, log = console.log } = {}) {
  log("building the library in memory (this takes ~20s for 200 assets) …");
  const { manifest, audio, errors, warnings, stats, packManifests } = prepareAudition();
  if (errors.length > 0) {
  ...
  log("");
  log(`  ♪  audition ${manifest.assetCount} assets at  http://127.0.0.1:${port}`);
  log("");
