...
export function applyFades(
	...
	const attack = Math.min(
		Math.floor(attackSeconds * SAMPLE_RATE),
		buffer.length,
	);
...
export function square(phase, frequency) {
	const limit = Math.min(harmonicCount(frequency), 64);
	let sum = 0;
	for (let h = 1; h <= limit; h += 2)
		sum += Math.sin(2 * Math.PI * h * phase) / h;
...
export function modal(
	length,
	{ fundamental, partials, amplitudeAt = () => 1 },
) {
	const out = new Float32Array(length);
	for (const partial of partials) {
		...
		for (let i = 0; i < length; i++) {
			const t = i / SAMPLE_RATE;
			out[i] +=
				Math.sin(step * i) * partial.gain * Math.exp(-t / partial.decay);
		}
	}
...
export function pluck(
	length,
	{ frequency, decay, damping = 0.5, blend = 1, rng },
) {
...
export function whiteNoise(length, rng, amplitudeAt = () => 1) {
	const out = new Float32Array(length);
	for (let i = 0; i < length; i++)
		out[i] = white(rng) * amplitudeAt(i / SAMPLE_RATE);
	return out;
}
...
function biquad(buffer, kind, cutoffAt, q) {
	...
	for (let i = 0; i < buffer.length; i++) {
		const cutoff = Math.min(
			Math.max(cutoffAt(i / SAMPLE_RATE), 10),
			SAMPLE_RATE * 0.48,
		);
...
export function trimTail(buffer, thresholdDbfs = -72, minimumSeconds = 0.02) {
	...
	const length = Math.max(
		minimum,
		Math.min(buffer.length, end + seconds(0.005)),
	);
