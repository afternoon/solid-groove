...
describe("loops (sample-library section 10)", () => {
	...
	it("computes the bar length from the tempo, not from the buffer", () => {
		...
		expect(loopFrames({ bpm: 120, bars: 1, timeSignature: "6/8" }).frames).toBe(
			72_000,
		);
	});
...
describe("presets (sample-library sections 5, 10)", () => {
	...
	it("covers a contiguous key range in every multisample instrument", () => {
		const instruments = allAssets.filter(
			(asset) => asset.preset?.kind === "instrument",
		);
		expect(instruments.length).toBeGreaterThan(0);
		for (const instrument of instruments) {
			const zones = [...instrument.preset.zones].sort(
				(a, b) => a.lowNote - b.lowNote,
			);
			for (let index = 1; index < zones.length; index++) {
				expect(zones[index].lowNote, instrument.id).toBe(
					zones[index - 1].highNote + 1,
				);
			}
		}
	});
	...
	it.each([
		...
		[
			...
			(m) => {
				m.assets.find(
					(a) => a.preset?.kind === "instrument",
				).preset.zones[0].rootNote = "C7";
			},
...
describe("derived masters (sample-library section 10)", () => {
	it("records the source asset, transform, and untouched checksum", () => {
		...
		for (const asset of derived) {
			...
			expect(asset.provenance.modifications).toContain(
				asset.derivedFrom.transform,
			);
