...
import { CHARACTERS, GENRES, INTENSITIES, SOURCE_TYPES, TAXONOMY } from "./taxonomy.mjs";
 ...
 */
export function prepareCandidates({ candidates: candidateList, lockfilePath } = {}) {
  ...
  const existing = new Map(
    readLockfile(lockfilePath).selections.map((selection) => [selection.id, selection]),
  );
...
export async function manage({ port = DEFAULT_PORT, log = console.log } = {}) {
  ...
  log("");
  log(`  ⚙  review ${candidates.length} candidates at  http://127.0.0.1:${port}`);
  log("");
