...
import { createManageServer, prepareCandidates, verifySelection } from "./manage.mjs";
...
describe("candidate config validation", () => {
  ...
  it("flags a duplicate candidate id", () => {
    ...
    expect(validateCandidates({ candidates: [one, { ...one }] }).join("\n")).toMatch(
      /duplicate id/,
    );
  });
...
describe("the committed candidate list", () => {
  ...
  it("carries a licenseConfidence on every entry", () => {
    for (const candidate of config.candidates) {
      expect(LICENSE_CONFIDENCE).toContain(candidate.licenseConfidence ?? "unconfirmed");
    }
  });
});
...
describe("draft selections", () => {
  ...
  it("requires a real reviewer, not the unreviewed sentinel", () => {
    expect(validateDraft({ ...draft(), reviewer: "unreviewed" }).join("\n")).toMatch(
      /real reviewer name/,
    );
  });
...
describe("verifySelection writes to the lockfile on disk", () => {
  ...
  it("rejects an invalid draft without writing", () => {
    const path = tempLockfile();
    expect(() => verifySelection({ ...draft(), creator: "" }, { path })).toThrow(
      /creator is required/,
    );
