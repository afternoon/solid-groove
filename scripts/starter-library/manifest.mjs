 ...
 */
const MODIFICATIONS = ["dc-offset-removal", "tail-trim", "peak-normalize", "edge-fades"];
...
export function buildLoop(entry, renderSource) {
  const samples = normalizePeak(renderLoop(entry, renderSource), entry.peakDbfs);
  const bytes = encodeWav(samples);
...
export function buildAllPacks(
  ...
  const packManifests = PACKS.filter((pack) => byPackId.has(pack.id)).map((pack) => {
