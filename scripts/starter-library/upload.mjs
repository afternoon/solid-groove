...
export function corsConfigFor(
  ...
  const raw = readFileSync(path, "utf8").replaceAll("{{PROJECT_ID}}", projectId);
...
export function planUpload() {
  ...
  for (const packManifest of packManifests) {
    ...
    errors.push(...result.errors.map((error) => `[${packManifest.pack.slug}] ${error}`));
    warnings.push(
      ...result.warnings.map((warning) => `[${packManifest.pack.slug}] ${warning}`),
    );
  }
  ...
  if (errors.length > 0) {
    ...
    throw new Error(`refusing to publish: manifest validation failed:\n${detail}`);
  }
...
function isTransient(error) {
  ...
  if (["ECONNRESET", "ETIMEDOUT", "ECONNREFUSED", "EPIPE", "EAI_AGAIN"].includes(code))
    return true;
  if (typeof code === "number" && (code === 429 || code >= 500)) return true;
  return /socket hang up|ECONNRESET|EPIPE|read ETIMEDOUT/i.test(error?.message ?? "");
}
...
export async function upload({
  ...
  const totalBytes = objects.reduce((sum, object) => sum + object.body.length, 0);
  ...
  log(`objects:  ${objects.length} (${(totalBytes / 1024 / 1024).toFixed(1)} MiB)`);
  ...
  if (configureBucket) {
    ...
    if (process.env.FIREBASE_STORAGE_EMULATOR_HOST) {
      ...
      log(`applied CORS: ${cors[0].method.join("/")} from ${cors[0].origin.join(", ")}`);
    }
  }
