...
describe("bucket resolution", () => {
	...
	it("fails with an actionable message when nothing is configured", () => {
		...
		expect(() => resolveBucketName(null)).toThrow(
	...
	it("derives the project id from either bucket naming scheme", () => {
		expect(projectIdFromBucket("solid-groove.firebasestorage.app")).toBe(
			"solid-groove",
		);
		expect(projectIdFromBucket("solid-groove.appspot.com")).toBe(
			"solid-groove",
		);
	});
});
...
describe("CORS configuration", () => {
	...
	it("admits a Hosting preview channel, whose subdomain cannot be enumerated", () => {
		const rules = corsConfigFor("solid-groove");
		expect(
			corsAllowsOrigin(rules, "https://solid-groove--pr-246-4wgdvx7q.web.app"),
		).toBe(true);
	});
	...
	it("matches an origin exactly, never by subdomain pattern", () => {
		...
		const named = [
			{ origin: ["https://solid-groove.web.app"], method: ["GET"] },
		];
		expect(corsAllowsOrigin(named, "https://solid-groove.web.app")).toBe(true);
		expect(
			corsAllowsOrigin(named, "https://solid-groove--pr-1-abc.web.app"),
		).toBe(false);
		expect(corsAllowsOrigin(named, "https://solid-groove.web.app", "PUT")).toBe(
			false,
		);
	});
...
describe("emulator environment", () => {
	it("derives STORAGE_EMULATOR_HOST, adding the scheme the client needs", () => {
		...
		expect(normalizeEmulatorEnv(env).STORAGE_EMULATOR_HOST).toBe(
			"http://127.0.0.1:9199",
		);
	});
...
describe("withRetry", () => {
	...
	it("does not retry a permission error", async () => {
		...
		await expect(withRetry(operation, noSleep)).rejects.toThrow(
			/permission denied/,
		);
		expect(operation).toHaveBeenCalledTimes(1);
	});
