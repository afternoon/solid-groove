 ...
 */
const ASSET_ID = new RegExp(`^sg-(${ASSET_TYPES.join("|")})-[a-z]+-[a-z-]+-\\d{4}$`);
...
export function validatePackManifest(
  ...
  if (packManifest?.schemaVersion !== 1) errors.push("manifest.schemaVersion must be 1");
...
function validatePackHeader(pack, errors) {
  ...
  if (!["factory", "user", "third-party"].includes(pack.kind)) {
    errors.push(`pack.kind must be factory, user, or third-party, not "${pack.kind}"`);
  }
...
function validateAsset(
  ...
  if (!asset?.id || !ASSET_ID.test(asset.id)) errors.push(`${where}: malformed asset id`);
  ...
  if (!asset?.pack?.id) {
    ...
    if (asset.license && pack.rights && asset.license.id !== pack.rights.licence) {
      errors.push(
  ...
  if (ASSET_ID.test(asset?.id ?? "") && !asset.id.startsWith(`sg-${asset.type}-`)) {
  ...
  if (!master) {
    ...
    if (!(master.bytes > 0)) errors.push(`${where}: files.master.bytes must be positive`);
    if (master.format !== expectedFormat) {
      errors.push(`${where}: unexpected format ${master.format} for a ${asset?.type}`);
    }
    if (
  ...
  if (asset?.type === "preset") {
    ...
    if (asset.waveform) errors.push(`${where}: a preset must not carry a waveform`);
    validatePreset(asset, where, errors);
  } else if (!audio) {
    errors.push(`${where}: audio metadata is required`);
  } else {
    if (audio.sampleRate !== 48000) errors.push(`${where}: sampleRate must be 48000`);
  ...
  if (!tags?.genres?.length) errors.push(`${where}: at least one genre tag is required`);
  if (!tags?.characters?.length)
    errors.push(`${where}: at least one character tag is required`);
  for (const genre of tags?.genres ?? []) {
    if (!GENRES.includes(genre)) errors.push(`${where}: unknown genre "${genre}"`);
  }
  ...
  if (!license) {
    ...
    if (!license.retrievedAt) errors.push(`${where}: license.retrievedAt is required`);
  ...
  if (!provenance) {
    ...
    if (provenance.sourceType === "synthesized" && asset?.type === "preset") {
      ...
      if (!provenance.recipe?.voice) {
        errors.push(`${where}: synthesized asset is missing its generation recipe`);
      }
      ...
      for (const field of [
        ...
        if (!provenance[field]) {
          errors.push(`${where}: acquired asset is missing provenance.${field}`);
        }
      }
      if (provenance.originalSha256 && !HEX_64.test(provenance.originalSha256)) {
...
function validateDerived(asset, where, errors) {
  ...
  if (!derivedFrom.assetId) errors.push(`${where}: derivedFrom.assetId is required`);
...
function validatePreset(asset, where, errors) {
  ...
  if (preset.kind !== asset.role) {
    errors.push(`${where}: preset kind "${preset.kind}" disagrees with its role`);
  }
  ...
  if (preset.kind === "drum-kit") {
    ...
    for (const pad of pads) {
      ...
      seen.add(pad.pad);
      if (!pad.assetId) errors.push(`${where}: pad ${pad.pad} references no asset`);
      if (/[/.]/.test(pad.assetId ?? "")) {
        errors.push(`${where}: pad ${pad.pad} references a path, not an asset ID`);
      }
    }
    return;
  }

  if (preset.kind === "instrument") validateInstrumentZones(preset, where, errors);
}
...
export function validateLibraryReferences(packManifests) {
  ...
  const check = (asset, reference, what) => {
    ...
    if (target.pack?.id !== asset.pack?.id) {
      ...
      errors.push(`${asset.id}: ${what} references ${reference} in a different pack`);
    }
  };
  ...
  for (const asset of assets) {
    ...
    if (asset.type === "derived" && asset.derivedFrom?.assetId) {
      ...
      if (source && source.files?.master?.sha256 !== asset.derivedFrom.sourceSha256) {
        errors.push(
...
function computeStats(assets) {
  ...
  for (const asset of assets) {
    ...
    for (const source of asset.tags?.sourceTypes ?? []) bump(stats.bySourceType, source);
    const characters = asset.tags?.characters ?? [];
    if (characters.some((c) => EXPERIMENTAL_CHARACTERS.includes(c))) experimental++;
...
function checkBalance(stats, total, { errors, warnings }) {
  ...
  const recorded =
    (stats.bySourceType.recorded ?? 0) + (stats.bySourceType["field-recording"] ?? 0);
...
export function validatePackIndex(index, packManifests, { serialized } = {}) {
  ...
  for (const pack of known.values()) {
    if (!seenIds.has(pack.id)) {
      errors.push(`pack "${pack.slug}" was built but is missing from the pack index`);
    }
  }
...
export function formatReport(stats, warnings) {
  ...
  if (stats.manifestGzipBytes) {
    lines.push(`manifest (gzipped):  ${(stats.manifestGzipBytes / 1024).toFixed(1)} KiB`);
  }
  lines.push(
