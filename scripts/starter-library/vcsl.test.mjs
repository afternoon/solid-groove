...
describe("VCSL instrument mapping", () => {
	it("maps mallets, bells, percussion, plucked and bowed strings", () => {
		...
		expect(mapVcslInstrument("Idiophones", "Tubular Bells 1").role).toBe(
			"bell",
		);
		expect(mapVcslInstrument("Membranophones", "Conga").role).toBe(
			"percussion",
		);
		expect(mapVcslInstrument("Chordophones", "Concert Harp").role).toBe(
			"pluck",
		);
...
describe("acquired asset IDs", () => {
	it("sit in a range disjoint from the lockfile ingest (base 5000)", () => {
		...
		expect(vcslAssetId("tonal", "mallet", 0)).toBe(
			"sg-one-shot-tonal-mallet-6001",
		);
	});
});
	...
	it("ingests the subset, writes a bundle, and never clones", async () => {
		...
		const run = (_cmd, argv) => {
			if (argv.includes("clone"))
				throw new Error("must not clone in this test");
