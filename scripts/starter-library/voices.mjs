...
function click(
	length,
	rng,
	{ amount = 0.5, decay = 0.002, cutoff = 3000 } = {},
) {
...
function snare(
	...
	return room > 0
		? reverb(driven, { time: 0.35, mix: room, damping: 0.5 })
		: driven;
}
...
function bell(
	{ note = "A5", decay = 2.2, index = 5, ratio = 1.41, strike = 0.3 },
	rng,
) {
	const frequency = noteToFrequency(note);
...
function bassStab(
	...
	const filtered = lowpass(
		body,
		(t) => cutoff * (0.35 + 0.65 * decayAt(t, decay, 1)),
		q,
	);
	return saturate(filtered, drive);
}
		...
		for (const direction of [-1, 1]) {
			layers.push({
				buffer: oscillate(length, {
					waveform,
					frequencyAt: () =>
						frequency * 2 ** ((direction * detuneCents) / 1200),
...
function sweepFx(
	...
	if (tone > 0) {
		const toneShape = (t) => {
			...
			const curved = rising ? progress ** 2 : 1 - (1 - progress) ** 2;
			return (
				toneStart * (toneEnd / toneStart) ** (rising ? curved : 1 - curved)
			);
		};
		layers.push({
			buffer: oscillate(length, {
...
function ambience(
	...
	const bed = pinkNoise(length, rng, (t) => {
		...
		const wander =
			0.6 +
			0.4 *
				Math.sin(2 * Math.PI * 0.037 * t) *
				Math.sin(2 * Math.PI * 0.011 * t + 1.3);
		return fade * wander;
	});
...
function reverseFx(
	{ source = "impact", duration = 1.6, tail = 0.06, params = {} },
	rng,
) {
	...
	for (let i = 0; i < punctuation; i++)
		out[reversed.length + i] += hit[i] * 0.5;
	return out;
}
