...
const LOOP_BAND_HEIGHT_PX = 10;

/**
 * The loop brace: a single band along the bottom of the ruler. A band rather
 * than a line so it cannot be mistaken for the playhead, and along the bottom
 * rather than the top so it cannot be mistaken for a section's colour strip.
 * Drawn before the labels, so the bar numbers stay readable above it.
 */
function drawLoopBrace(env: DrawEnvironment, loop: LoopBraceDrawState): void {
  ...
  ctx.fillStyle = loop.enabled ? colors().loopBrace : colors().loopBraceOff;
  ctx.fillRect(
    left,
