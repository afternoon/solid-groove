...
describe("edges: handles versus body", () => {
  ...
  it("switches from handle to body one tick past the handle width", () => {
    ...
    expect(insideHandle.kind === "placement" ? insideHandle.handle : null).toBe("start");
    ...
    expect(pastHandle.kind === "placement" ? pastHandle.handle : null).toBe("body");
  });
...
describe("zoom extremes", () => {
  ...
  it("still finds the body at maximum zoom-in, where a bar is very wide", () => {
    ...
    const grown = apply(
      base,
      resizePlacement(base, base.song.placements[0].id, "end", TICKS_PER_BAR * 4),
    );
    const projection = projectionOf(grown);
    const [placement] = [...projection.placementsById.values()];
...
describe("offscreen culling", () => {
  it("excludes a placement far to the right of the viewport", () => {
    const project = withPlacementAt(createSliceFixtureProject(), TICKS_PER_BAR * 80);
    ...
    const visible = visiblePlacementsForTrack(index, visibleTickRange(viewport, 200));
  ...
  it("a hit test on an offscreen row costs nothing and reports empty", () => {
    const projection = projectionOf(createSliceFixtureProject());
    expect(
      hitTestArrangement(projection, { rowIndex: -1, tick: 0 }, handleTicks(0.08)),
    ).toEqual({ kind: "empty" });
  });
});
...
describe("ten-minute bounds (PRD ARR-01)", () => {
  ...
  it("spans at least ten minutes at every supported tempo", () => {
    ...
    expect(ticksToSeconds(MAX_ARRANGEMENT_TICKS, SONG_TEMPO.max)).toBeGreaterThanOrEqual(
      600,
    );
    expect(
  ...
  it("moves a placement out to minute nine at the default tempo", () => {
    ...
    expect(ticksToSeconds(moved.song.placements[0].startTicks, 120)).toBeCloseTo(540, 0);
  });
  ...
  it("hit-tests a placement at the far end of the guaranteed range", () => {
    ...
    expect(result.kind === "placement" ? result.placementId : null).toBe(placement.id);
  });
