...
import { SAMPLER_PITCH, SYNTH_FILTER_CUTOFF, SYNTH_WAVEFORM } from "../domain/parameters";
...
describe("createInstrumentNode", () => {
  it("sampler: triggers nothing until its asset buffer resolves", async () => {
    ...
    const node = InstrumentGraphModule.createInstrumentNode(instrument, context);
    ...
    expect(() => node.trigger({ kind: "pitch", pitch: 60 }, 0, "8n", 0.8)).not.toThrow();
  ...
  describe("drumMachine playback", () => {
    ...
    async function realBufferContext(scope: import("./AudioRuntime").AudioProjectScope) {
...
describe("playOneShot", () => {
  it("starts the player at the requested offset inside the buffer", async () => {
    ...
    try {
      InstrumentGraphModule.playOneShot(buffer, destination, 0, "192i", 1, 0.25);
  ...
  it("survives parameter extremes and repeated triggers without throwing", async () => {
    ...
    expect(() => {
      for (let i = 0; i < 64; i += 1) {
        node.trigger({ kind: "pitch", pitch: 36 + (i % 48) }, i * 0.01, 0.05, 1);
      }
    }).not.toThrow();
    node.dispose();
    await runtime.close();
...
describe("one-shot sampler (PRD INS-01)", () => {
  ...
  it("a parameter edit keeps the same subscription and disposes cleanly", async () => {
    ...
    expect(runtime.diagnostics().resources.byType.subscription).toBe(subscriptions);
