...
describe("playAudioLoop", () => {
  it("stretches with a GrainPlayer at an untouched pitch when the tempo differs", () => {
    ...
    expect(player.grainSize).toBeCloseTo(audioLoopPlayer.LOOP_GRAIN_SIZE_SECONDS);
    expect(player.overlap).toBeCloseTo(audioLoopPlayer.LOOP_GRAIN_OVERLAP_SECONDS);
