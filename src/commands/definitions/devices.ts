...
function readChain(
  ...
  switch (target.chain) {
    ...
    case "return":
      return project.song.returns.find((bus) => bus.id === target.returnId)?.devices;
    case "master":
    ...
    case "insert": {
      ...
      return withSong(project, {
        ...
        tracks: project.song.tracks.map((candidate) =>
          candidate.id === track.id ? { ...candidate, devices: [...devices] } : candidate,
        ),
      });
    }
    case "return":
...
export type DeviceDuplicatePayload = z.infer<typeof deviceDuplicatePayloadSchema>;
...
export type DeviceSetBypassPayload = z.infer<typeof deviceSetBypassPayloadSchema>;
...
export const deviceAddCommand = defineCommand<DeviceAddPayload>({
  ...
  apply(project, payload) {
    ...
    if (payload.target.chain === "insert" && devices.length >= MAX_TRACK_INSERTS) {
      return rejected(`A track may have at most ${MAX_TRACK_INSERTS} insert devices`);
    }
...
export const deviceReorderCommand = defineCommand<DeviceReorderPayload>({
  ...
  invert(payload, before) {
    ...
    return device ? [reorderDevice(payload.target, payload.deviceId, device.order)] : [];
  },
});
...
export const deviceSetBypassCommand = defineCommand<DeviceSetBypassPayload>({
  ...
  apply(project, payload) {
    ...
    const next = devices.map((device) =>
      device.id === payload.deviceId ? { ...device, bypassed: payload.bypassed } : device,
    );
    return applied(writeChain(project, payload.target, next));
  },
  invert(payload, before) {
...
export const deviceResetCommand = defineCommand<DeviceResetPayload>({
  ...
  invert(payload, before) {
    ...
    return device
      ? [restoreDeviceParameters(payload.target, payload.deviceId, device.parameters)]
      : [];
  },
});
...
export type DeviceResetRestorePayload = z.infer<typeof deviceResetRestorePayloadSchema>;

export const deviceResetRestoreCommand = defineCommand<DeviceResetRestorePayload>({
