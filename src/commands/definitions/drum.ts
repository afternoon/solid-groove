...
export type DrumSetPadAssetPayload = z.infer<typeof drumSetPadAssetPayloadSchema>;
...
export const drumSetPadAssetCommand = defineCommand<DrumSetPadAssetPayload>({
  ...
  invert(payload, before) {
    ...
    const pad = findPad(track.instrument.pads, payload.padId);
    return pad ? [setPadAsset(payload.trackId, payload.padId, pad.assetId)] : [];
  },
});
...
export type DrumSetPadChokePayload = z.infer<typeof drumSetPadChokePayloadSchema>;
...
export const drumSetPadChokeCommand = defineCommand<DrumSetPadChokePayload>({
  ...
  invert(payload, before) {
    ...
    if (track?.instrument?.kind !== "drumMachine") return [];
    const pad = findPad(track.instrument.pads, payload.padId);
    return pad ? [setPadChoke(payload.trackId, payload.padId, pad.chokeGroup)] : [];
  },
});
...
export type DrumSetPadParameterPayload = z.infer<typeof drumSetPadParameterPayloadSchema>;
...
function padParameterKey(parameterId: string): string {
  return parameterId.startsWith("pad.") ? parameterId.slice("pad.".length) : parameterId;
}

function currentPadParameter(pad: DrumPad, definition: ParameterDefinition): number {
  const field = padMixerField(definition.id);
  if (field) return pad.mixer[field];
  return pad.parameters[padParameterKey(definition.id)] ?? definition.defaultValue;
}
...
export const drumSetPadParameterCommand = defineCommand<DrumSetPadParameterPayload>({
...
function restoreClipHits(
  ...
  const merged = [...existing, ...events].sort((a, b) => a.startTicks - b.startTicks);
...
export const drumRemovePadCommand = defineCommand<DrumRemovePadPayload>({
  ...
  invert(payload, before) {
    const track = findTrack(before, payload.trackId);
    if (track?.instrument?.kind !== "drumMachine") return [];
    const index = track.instrument.pads.findIndex((pad) => pad.id === payload.padId);
    if (index < 0) return [];
...
export const drumReorderPadCommand = defineCommand<DrumReorderPadPayload>({
  ...
  apply(project, payload) {
    ...
    const from = track.instrument.pads.findIndex((pad) => pad.id === payload.padId);
    if (from < 0) {
      return rejected(`Pad ${payload.padId} does not exist`);
    }
