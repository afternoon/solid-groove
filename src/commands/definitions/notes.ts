...
export const noteUpdatePayloadSchema = z.strictObject({
  clipId: clipIdSchema,
  updates: z
    .array(z.strictObject({ eventId: eventIdSchema, changes: noteChangesSchema }))
    .min(1),
});
...
export const noteRemoveCommand = defineCommand<NoteRemovePayload>({
  ...
  apply(project, payload) {
    ...
    if (missing.length > 0) {
      return rejected(`Clip ${clip.id} has no note ${missing.join(", ")} to delete`);
    }
    return applied(
      replaceClip(
        project,
  ...
  invert(payload, before) {
    ...
    const removed = events.filter((event) => payload.eventIds.includes(event.id));
...
export const noteUpdateCommand = defineCommand<NoteUpdatePayload>({
  ...
  apply(project, payload) {
    ...
    for (const update of payload.updates) {
      const source = changed.get(update.eventId) ?? findNoteEvent(clip, update.eventId);
      if (!source) {
        return rejected(`Clip ${clip.id} has no note ${update.eventId} to edit`);
      }
...
export function applyNoteChanges(event: NoteEvent, changes: NoteChanges): NoteEvent {
  return {
    ...event,
    ...(changes.trigger !== undefined ? { trigger: changes.trigger } : {}),
    ...(changes.startTicks !== undefined ? { startTicks: changes.startTicks } : {}),
    ...(changes.durationTicks !== undefined
      ? { durationTicks: changes.durationTicks }
      : {}),
    ...(changes.velocity !== undefined ? { velocity: changes.velocity } : {}),
    ...(changes.probability !== undefined ? { probability: changes.probability } : {}),
  };
}
...
function previousValues(event: NoteEvent, changes: NoteChanges): NoteChanges {
  return {
    ...
    ...(changes.startTicks !== undefined ? { startTicks: event.startTicks } : {}),
    ...(changes.durationTicks !== undefined
      ? { durationTicks: event.durationTicks }
      : {}),
    ...(changes.velocity !== undefined ? { velocity: event.velocity } : {}),
    ...(changes.probability !== undefined ? { probability: event.probability } : {}),
  };
}
