...
describe("device.remove and device.reorder renumber contiguously", () => {
  it("removes a device and closes the order gap", () => {
    ...
    expect(devices.map((d) => d.order)).toEqual(devices.map((_, index) => index));
  });
...
describe("insert ceiling (FX-01)", () => {
  ...
  it("accepts inserts up to the ceiling but rejects one past it", () => {
    ...
    for (let order = startingInserts; order < MAX_TRACK_INSERTS; order++) {
      project = apply(
        project,
        addDevice(insertChain(trackId), createDevice(newDeviceId(), "filter", order)),
      );
    }
...
describe("device.duplicate", () => {
  it("places an independent copy right after the source with a fresh id", () => {
    ...
    let project = apply(
      fixture.project,
      addDevice(insertChain(fixture.trackAId), createDevice(sourceId, "delay", 2)),
    );
    project = apply(
      project,
      setParameter(
    ...
    expect(devices[copyIndex].parameters).toEqual(devices[sourceIndex].parameters);
    expect(devices[copyIndex].parameters).not.toBe(devices[sourceIndex].parameters);
    expect(devices.map((d) => d.order)).toEqual(devices.map((_, index) => index));
  });
});
...
describe("device.setBypass and device.reset", () => {
  it("bypasses a device without touching its parameters", () => {
    ...
    const device = trackDevices(next, fixture).find((d) => d.id === fixture.deviceId);
  ...
  it("reset restores factory parameters but keeps position, bypass, and preset", () => {
    ...
    project = apply(project, setDeviceBypass(insertChain(fixture.trackAId), id, true));
    const reset = apply(project, resetDevice(insertChain(fixture.trackAId), id));
...
describe("unrelated graphs keep identity (structural sharing)", () => {
  it("adding a device to track A leaves track B's object referentially identical", () => {
    const fixture = createCommandTestProject();
    const before = fixture.project.song.tracks.find((t) => t.id === fixture.trackBId);
    const next = apply(
      fixture.project,
      addDevice(insertChain(fixture.trackAId), createDevice(newDeviceId(), "reverb", 2)),
    );
