...
describe("drum-machine commands", () => {
	...
	describe("drum.setPadAsset", () => {
		it("replaces a pad's sample without touching the pad's identity or its other fields", () => {
			...
			const pad = drumPadsOf(next, fixture.trackBId).find(
				(p) => p.id === kickId,
			);
			...
			const hitsBefore =
				beatClip?.content.kind === "notes"
					? beatClip.content.events.filter(
							(event) =>
								event.trigger.kind === "pad" && event.trigger.padId === kickId,
						)
					: [];
		...
		it("can clear a pad back to no sample", () => {
			...
			const next = apply(
				fixture.project,
				setPadAsset(fixture.trackBId, kickId, null),
			);
			expect(
				drumPadsOf(next, fixture.trackBId).find((pad) => pad.id === kickId)
					?.assetId,
			).toBeNull();
		});
	...
	describe("drum.setPadFlag mute/solo", () => {
		it("sets and clears pad mute and solo independently", () => {
			...
			const soloed = apply(
				muted,
				setPadFlag(fixture.trackBId, kickId, "soloed", true),
			);
			const pad = drumPadsOf(soloed, fixture.trackBId).find(
				(p) => p.id === kickId,
			);
	...
	describe("drum.setPadChoke", () => {
		it("assigns and removes a choke group", () => {
			...
			const grouped = apply(
				fixture.project,
				setPadChoke(fixture.trackBId, kickId, 2),
			);
			expect(
				drumPadsOf(grouped, fixture.trackBId).find((p) => p.id === kickId)
					?.chokeGroup,
			).toBe(2);
			const cleared = apply(
				grouped,
				setPadChoke(fixture.trackBId, kickId, null),
			);
			expect(
				drumPadsOf(cleared, fixture.trackBId).find((p) => p.id === kickId)
					?.chokeGroup,
			).toBeNull();
		});
	...
	describe("drum.addPad / drum.removePad / drum.reorderPad", () => {
		...
		it("rejects adding a pad whose ID already exists", () => {
			...
			const result = executeCommand(
				fixture.project,
				addPad(fixture.trackBId, existing),
			);
			expect(result.ok).toBe(false);
		});
		...
		it("cascades to remove the hits that triggered the removed pad, and undo restores them exactly", () => {
			...
			expect(history.execute(removePad(fixture.trackBId, kickId)).ok).toBe(
				true,
			);

			const afterClip = history.project.clips.find(
				(clip) => clip.id === beatClip?.id,
			);
			const hitsAfter =
			...
			const restoredClip = history.project.clips.find(
				(clip) => clip.id === beatClip?.id,
			);
			expect(
				restoredClip?.content.kind === "notes"
					? restoredClip.content.events
					: [],
			).toEqual(hitsBefore);
			expect(
				drumPadsOf(history.project, fixture.trackBId).map((pad) => pad.id),
			).toEqual(fixture.padIds);
		});
		...
		it("reorders a pad while preserving every pad's identity", () => {
			...
			const next = apply(
				fixture.project,
				reorderPad(fixture.trackBId, kickId, 1),
			);
	...
	describe("save / reload", () => {
		it("round-trips every edited pad field through serialize + parse", () => {
			...
			const pad = drumPadsOf(reloaded, fixture.trackBId).find(
				(p) => p.id === kickId,
			);
			expect(pad?.mixer.muted).toBe(true);
			expect(pad?.mixer.soloed).toBe(true);
