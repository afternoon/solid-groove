...
const cases: InverseCase[] = [
  ...
  },
  {
    ...
    build: (fixture) => setPadFlag(fixture.trackBId, fixture.padIds[0], "soloed", true),
  },
  {
  ...
  },
  {
    ...
    build: (fixture) => changeInstrument(fixture.trackAId, createSynthInstrument()),
  },
  {
  ...
  },
  {
    ...
    build: (fixture) =>
      addDevice(
        ...
        createDevice(createSeededIdFactory("inverse-device-add")("device"), "reverb", 1),
      ),
  },
  {
    type: "device.remove",
    build: (fixture) => removeDevice(insertChain(fixture.trackAId), fixture.deviceId),
  },
  {
    type: "device.reorder",
    // The fixture's track A has two inserts; move the first past the second.
    build: (fixture) => reorderDevice(insertChain(fixture.trackAId), fixture.deviceId, 1),
  },
  {
  ...
  {
    type: "device.reset",
    build: (fixture) => resetDevice(insertChain(fixture.trackAId), fixture.deviceId),
  },
  {
...
describe("generated inverses", () => {
  it("covers every registered command", () => {
    expect(cases.map((entry) => entry.type).sort()).toEqual([...COMMAND_TYPES].sort());
  });
