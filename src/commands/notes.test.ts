...
describe("note commands", () => {
  ...
  describe("note.add", () => {
    it("adds a note to a note clip", () => {
      const result = executeCommand(fixture.project, addNotes(fixture.clipAId, [note]));
    ...
    it("rejects a well-formed but unknown clip ID", () => {
      const result = executeCommand(fixture.project, addNotes(ABSENT_IDS.clip, [note]));
    ...
    it("rejects an audio-loop clip", () => {
      ...
      if (!loopClip) return;
      const result = executeCommand(audioProject, addNotes(loopClip.id, [note]));
      expect(result.ok).toBe(false);
      if (result.ok) return;
    ...
    it("rejects an empty note list at the schema", () => {
      const result = executeCommand(fixture.project, addNotes(fixture.clipAId, []));
      expect(result.ok).toBe(false);
      if (result.ok) return;
  ...
  describe("note.update", () => {
    ...
    it("clears a probability with an explicit null", () => {
      ...
      expect(eventsOf(withProbability.project, fixture.clipAId)[0].probability).toBe(0.5);
      ...
      expect(eventsOf(cleared.project, fixture.clipAId)[0].probability).toBe(null);
    });
