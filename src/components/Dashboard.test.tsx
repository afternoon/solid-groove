...
describe("Dashboard", () => {
  it("shows a retry affordance when the project listing errors", async () => {
    ...
    expect(
      await screen.findByText("Something went wrong while loading your projects."),
    ).toBeInTheDocument();
  ...
  it("surfaces a message near New Project when project creation fails", async () => {
    ...
    expect(
      await screen.findByText("Couldn't create a new project. Please try again."),
    ).toBeInTheDocument();
  ...
  describe("rename", () => {
    it("renames a project and reflects the new name", async () => {
      ...
      expect(saveMetadata).toHaveBeenCalledWith("prj_abc", { name: "Renamed Groove" }, 3);
    });
  ...
  describe("delete", () => {
    ...
    it("deletes after confirmation and logs project_deleted", async () => {
      ...
      await vi.waitFor(() => expect(deleteProject).toHaveBeenCalledWith("prj_abc"));
      expect(await screen.findByText("No projects yet")).toBeInTheDocument();
