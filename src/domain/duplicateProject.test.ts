...
describe("duplicateProject", () => {
  ...
  it("mints independent IDs for every track, device, return, section, clip, event, placement, automation lane, and asset", () => {
    ...
    expect(duplicate.song.placements).toHaveLength(source.song.placements.length);
    expect(duplicate.song.automation).toHaveLength(source.song.automation.length);
  ...
  it("keeps pack references (packId/packVersion) unchanged on duplicated assets", () => {
    ...
    const sourcePacks = source.song.assets.map((a) => [a.packId, a.packVersion]);
    const duplicatePacks = duplicate.song.assets.map((a) => [a.packId, a.packVersion]);
    expect(duplicatePacks).toEqual(sourcePacks);
    expect(duplicate.metadata.packDependencies).toEqual(source.metadata.packDependencies);
  });
  ...
  it("remaps the sampler instrument's asset reference to the duplicated asset", () => {
    ...
    if (
      ...
      expect(duplicate.song.assets.some((a) => a.id === duplicateAssetId)).toBe(true);
    }
  });
  ...
  it("remaps the audio-loop clip's asset reference", () => {
    ...
    const sourceLoopClip = source.clips.find((c) => c.content.kind === "audioLoop");
    const duplicateLoopClip = duplicate.clips.find((c) => c.content.kind === "audioLoop");
    if (
  ...
  it("remaps every automation target scope (track, trackDevice, send, return, master)", () => {
    ...
    const sourceScopes = source.song.automation.map((lane) => lane.target.scope);
    const duplicateScopes = duplicate.song.automation.map((lane) => lane.target.scope);
    ...
    for (const lane of duplicate.song.automation) {
      ...
      if (
        ...
        expect(duplicate.song.tracks.some((t) => t.id === target.trackId)).toBe(true);
      }
      if (target.scope === "trackDevice") {
        const track = duplicate.song.tracks.find((t) => t.id === target.trackId);
        expect(track?.devices.some((d) => d.id === target.deviceId)).toBe(true);
      }
      if (target.scope === "send" || target.scope === "return") {
        expect(duplicate.song.returns.some((r) => r.id === target.returnId)).toBe(true);
      }
    }
  });
  ...
  it("remaps the track send's return reference", () => {
    ...
    const duplicateTrack = duplicate.song.tracks.find((t) => t.sendConfig.length > 0);
    ...
    expect(
      duplicate.song.returns.some((r) => r.id === duplicateTrack?.sendConfig[0].returnId),
    ).toBe(true);
  });
