...
export function duplicateTrack(
  ...
  const source = project.song.tracks.find((track) => track.id === sourceTrackId);
  if (!source) {
  ...
  const automation: AutomationLane[] = project.song.automation
    .filter((lane) => "trackId" in lane.target && lane.target.trackId === source.id)
    .map((lane) => ({
      ...lane,
...
function remapTrigger(trigger: NoteTrigger, padIds: Map<PadId, PadId>): NoteTrigger {
  if (trigger.kind === "pad") {
...
function remap<K>(map: Map<K, K>, key: K): K {
  ...
  if (next === undefined) {
    throw new Error("duplicateTrack: reference resolved before its owner was remapped");
  }
  return next;
}
