...
describe("blank project factory", () => {
  ...
  it("keeps Firebase and audio types out of the domain", () => {
    ...
    expect(JSON.parse(JSON.stringify(project)).metadata.createdAt).toBe(1_700_000);
  });
  ...
  it("clamps an out-of-range tempo through the shared parameter definition", () => {
    expect(createBlankProject({ ownerId: "user_1", tempo: 5_000 }).song.tempo).toBe(
      SONG_TEMPO.max,
    );
  });
...
describe("fixtures", () => {
  ...
  it("adds audio tracks with audioLoop clips when waveformTrackCount is set", () => {
    ...
    const audioTracks = project.song.tracks.filter((track) => track.type === "audio");
    expect(audioTracks).toHaveLength(4);
    expect(audioTracks.every((track) => track.instrument === null)).toBe(true);

    const clipsByTrack = new Map(project.clips.map((clip) => [clip.trackId, clip]));
    ...
    for (const assetId of audioLoopAssetIds) {
      expect(project.song.assets.some((asset) => asset.id === assetId)).toBe(true);
    }
  });
  ...
  describe("packs", () => {
    ...
    it("keeps every fixture's dependency list derived, not hand-written", () => {
      for (const project of [
        ...
        expect(project.metadata.packDependencies, project.metadata.name).toEqual(
          derivePackDependencies(project.song),
        );
      }
    });
...
describe("FND-008 arrangement spike fixtures", () => {
  it.each(ARRANGEMENT_BENCHMARK_TRACK_COUNTS)(
    ...
    (trackCount) => {
      ...
      expect(project.song.placements.length).toBeGreaterThanOrEqual(trackCount * 50);
  ...
  it("is deterministic across calls at the same track count", () => {
    expect(createLargeArrangementProject(20)).toEqual(createLargeArrangementProject(20));
  });
