...
describe("perceptual fader curve", () => {
  ...
  it("clamps out-of-range positions rather than mapping past the fader", () => {
    ...
    expect(faderPositionToDb(TRACK_VOLUME, -1)).toBeCloseTo(TRACK_VOLUME.min, 5);
    expect(faderPositionToDb(TRACK_VOLUME, Number.NaN)).toBeCloseTo(TRACK_VOLUME.min, 5);
  });
  ...
  it("works for any decibel parameter, not just track volume", () => {
    ...
    expect(faderPositionToDb(MASTER_VOLUME, 1)).toBeCloseTo(MASTER_VOLUME.max, 5);
  });
});
