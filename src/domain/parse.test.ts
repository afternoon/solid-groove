...
import { createDrumMachineFixtureProject, createSliceFixtureProject } from "./fixtures";
...
function audioLoopClip(project: MutableProject): MutableClip {
  const clip = project.clips.find((entry) => entry.content.kind === "audioLoop");
  if (!clip) {
...
describe("parseProject", () => {
  ...
  it("rejects automation targeting a missing track, unknown parameter, or fixed parameter", () => {
    ...
    unknownParameter.song.automation = [
      lane(ids("automation"), unknownParameter.song.tracks[0].id, "track.mystery"),
    ];
...
describe("pack-qualified asset identity", () => {
  it("accepts a fixture whose assets span two packs at two versions", () => {
    ...
    expect(
      new Set(input.metadata.packDependencies.map((entry) => entry.version)).size,
    ).toBe(2);
  });
  ...
  it("rejects a project that depends on two versions of one pack", () => {
    ...
    input.metadata.packDependencies = [first, { packId: first.packId, version: "2.0.0" }];
    expectIssue(parseProject(input), "invalid_pack_reference");
  });
...
describe("pack shelf (addedPacks, LIB-08)", () => {
  ...
  it("rejects a shelf that lists a used pack at the wrong version", () => {
    ...
    input.metadata.addedPacks = input.metadata.addedPacks.map((entry) =>
      entry.packId === used.packId ? { packId: entry.packId, version: "9.9.9" } : entry,
    );
    expectIssue(parseProject(input), "invalid_pack_reference");
  });
...
describe("persistence tier parsers", () => {
  ...
  it("refuses a future schema version on the metadata document", () => {
    ...
    expectIssue(parseProjectMetadata(input.metadata), "unsupported_schema_version");
  });
});
...
function lane(id: string, trackId: string, parameterId: string): MutableAutomation {
  return {
    id,
