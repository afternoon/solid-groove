...
import { checkAddedPacks, checkMetadata, checkPackQualification } from "./packs";
import {
  claimId,
  type DomainIssue,
 ...
 */
export function parseProjectMetadata(input: unknown): ParseResult<ProjectMetadata> {
...
export function checkProjectIntegrity(project: Project): DomainIssue[] {
  ...
  const trackIds = new Map(project.song.tracks.map((track) => [track.id, track]));
