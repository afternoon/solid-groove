...
export function serializeProjectMetadata(metadata: ProjectMetadata): JsonObject {
  return {
    id: metadata.id,
    ...
    addedPacks: sortBy(metadata.addedPacks, (entry) => [entry.packId, entry.version]).map(
      (entry) => ({
        packId: entry.packId,
...
export function serializeSong(song: Song): JsonObject {
  return {
    ...
    automation: sortBy(song.automation, (lane) => [automationSortKey(lane), lane.id]).map(
      serializeAutomationLane,
    ),
...
function serializeTrack(track: Track): JsonObject {
  return {
    ...
    sendConfig: sortBy(track.sendConfig, (send) => [send.returnId]).map(serializeSend),
    mixer: {
...
function serializeDevices(devices: readonly Device[]): JsonValue[] {
  return sortBy(devices, (device) => [device.order, device.id]).map(serializeDevice);
}
...
function serializeAutomationTarget(target: AutomationLane["target"]): JsonObject {
  switch (target.scope) {
    case "track":
      return {
        scope: target.scope,
...
function serializeParameters(parameters: Readonly<Record<string, number>>): JsonObject {
