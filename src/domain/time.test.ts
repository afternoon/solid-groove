...
describe("musical time", () => {
  ...
  it("converts bars, beats, and sixteenths without losing ticks", () => {
    ...
    expect(barsBeatsSixteenthsToTicks({ bars: 1, beats: 2, sixteenths: 3 })).toBe(
      TICKS_PER_BAR + 2 * TICKS_PER_QUARTER + 3 * TICKS_PER_SIXTEENTH,
    );
  });

  it("formats and parses Tone.js bars:beats:sixteenths", () => {
    expect(formatBarsBeatsSixteenths(0)).toBe("0:0:0");
    expect(formatBarsBeatsSixteenths(TICKS_PER_BAR + TICKS_PER_SIXTEENTH)).toBe("1:0:1");
    expect(formatBarsBeatsSixteenths(24)).toBe("0:0:0.5");
    expect(parseBarsBeatsSixteenths("1:0:1")).toBe(TICKS_PER_BAR + TICKS_PER_SIXTEENTH);
    expect(() => parseBarsBeatsSixteenths("1:0")).toThrow(/bars:beats:sixteenths/);
    expect(() => parseBarsBeatsSixteenths("a:b:c")).toThrow(/Invalid musical position/);
  });
