...
function assertTempo(bpm: number): void {
  if (!Number.isFinite(bpm) || bpm <= 0) {
    throw new TypeError(`Tempo must be a positive finite number, received ${bpm}`);
  }
}
...
export function parseBarsBeatsSixteenths(value: string): Ticks {
  ...
  if (parts.length !== 3) {
    throw new TypeError(`Expected "bars:beats:sixteenths", received "${value}"`);
  }
  ...
  return roundToTicks(
    bars * TICKS_PER_BAR + beats * TICKS_PER_QUARTER + sixteenths * TICKS_PER_SIXTEENTH,
  );
}
