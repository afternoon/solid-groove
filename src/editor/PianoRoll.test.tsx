...
describe("PianoRoll", () => {
  ...
  // #255: the per-note velocity slider was removed — the note block is too
  // cramped for a usable control. Its absence must not cost the note its
  // velocity, and moving the note must carry that velocity along.
  it("renders no velocity slider in a note block and keeps the note's velocity (#255)", async () => {
    const { session, renderRoll } = await setUp();
    const { container } = renderRoll();
    const note = noteEvents(session.project.clips[0])[0];
    const el = noteEl(container, note.id);
    ...
    expect(container.querySelector('[aria-label^="Velocity of "]')).toBeNull();
    ...
    const moved = noteEvents(session.project.clips[0]).find((n) => n.id === note.id);
    expect(moved?.startTicks).toBe(note.startTicks + TICKS_PER_SIXTEENTH);
    expect(moved?.velocity).toBe(note.velocity);
  });
  ...
  describe("analytics (PRD OPS-02)", () => {
    ...
    it("emits the piano_roll feature_first_use exactly once across many actions", async () => {
      ...
      // A create, then a move — two distinct actions.
      firePointer(contentEl(container), "pointerdown", {
        button: 0,
