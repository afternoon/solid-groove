...
export default function PianoRoll(props: PianoRollProps): JSX.Element {
  ...
  const [pixelsPerTick, setPixelsPerTick] = createSignal(DEFAULT_PIXELS_PER_TICK);
  ...
  const [keyRoot, setKeyRoot] = createSignal<PitchClass>(props.keyGuide?.root ?? 0);
  const [keyMode, setKeyMode] = createSignal<ScaleMode>(props.keyGuide?.mode ?? "major");
  ...
  function createNoteAt(clientX: number, clientY: number): void {
    const view = viewport();
    const startTicks = snapTicksFloor(clientXToTick(view, contentLeft(), clientX));
  ...
  function beginDrag(
    ...
    const originals = noteEvents(props.clip).filter((candidate) => ids.has(candidate.id));
      ...
      <div class="piano-roll-body">
        ...
        >
          ...
          <Show when={marqueeStyle()}>
            {(style) => <div class="pr-marquee" style={style()} aria-hidden="true" />}
          </Show>
