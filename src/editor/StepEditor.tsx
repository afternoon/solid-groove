...
export default function StepEditor(props: StepEditorProps): JSX.Element {
	...
	const setSelectedIds = (
		next:
			| readonly EventId[]
			| ((prev: readonly EventId[]) => readonly EventId[]),
	): void => {
	...
	function resizeToBars(nextBars: number): void {
		const clamped = Math.min(
			MAX_BARS,
			Math.max(MIN_BARS, Math.round(nextBars)),
		);
			...
			<div class="step-editor-toolbar">
				<label class="step-editor-length">
					...
					<select
						...
						value={bars()}
						onChange={(event) =>
							resizeToBars(Number(event.currentTarget.value))
						}
					>
						<For
							each={Array.from(
									...
									{(step) => {
										...
										return (
											<button
												...
												onPointerDown={(event) =>
													onCellPointerDown(event, lane, step)
												}
...
export function deleteSelectedNotes(
	...
	const present = new Set(
		clip.content.kind === "notes"
			? clip.content.events.map((event) => event.id)
			: [],
	);
