 ...
 */
export default function LibraryBrowser(props: LibraryBrowserProps): JSX.Element {
...
function PackNode(props: {
  ...
  const summary = createMemo(
    () => props.browser.packs().find((pack) => pack.slug === props.pack.slug) ?? null,
  );
  ...
  return (
    <li class="library-tree-pack">
      ...
      >
        ...
        <span class="library-node-count">{props.pack.failed ? "!" : count()}</span>
      </button>
      <Show when={expanded()}>
        <Show
        ...
        >
          <Show when={props.pack.loaded} fallback={<TapeLoader label="Loading pack" />}>
            <Show
