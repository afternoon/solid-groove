...
describe("browsing packs", () => {
  it("lists every pack with what it is and who made it", async () => {
    ...
    expect(within(list).getByRole("button", { name: /All sounds/ })).toBeInTheDocument();
  });
  ...
  it("searches packs by name without fetching their manifests", async () => {
    ...
    fireEvent.input(within(dialog).getByRole("searchbox", { name: "Search packs" }), {
  ...
  it("says so when no pack matches", async () => {
    ...
    expect(await within(dialog).findByText(/No packs match/)).toBeInTheDocument();
  });
});
...
describe("the pack detail view", () => {
  it("shows the pack's own description and contents", async () => {
    ...
    expect(within(detail).getByText(SECOND_PACK.description)).toBeInTheDocument();
    await waitFor(() =>
      expect(
...
describe("the cross-pack view (LIB-02)", () => {
  it("searches and filters across every loaded pack, and clearing re-exposes", async () => {
    ...
    fireEvent.input(within(dialog).getByRole("searchbox", { name: "Search sounds" }), {
      target: { value: "kick" },
    });
    await waitFor(() =>
      expect(
    ...
    await waitFor(() =>
      expect(within(dialog).getAllByRole("button", { name: /^Audition / }).length).toBe(
        total,
      ),
    );
  });
  ...
  it("names an unavailable pack while the rest stay browsable", async () => {
    ...
    expect(within(dialog).getByText(/Other packs still work/)).toBeInTheDocument();
    expect(
      within(dialog).getAllByRole("button", { name: /^Audition / }).length,
    ).toBeGreaterThan(0);
  });
