...
function PackDetail(props: {
  ...
  const failed = createMemo(() =>
    props.browser.packErrors().some((error) => error.packSlug === props.pack.slug),
  );
  return (
    <>
      <div class="pack-detail">
        ...
        <div class="pack-detail-hero">
          ...
          <div class="pack-detail-heading">
            ...
            <p class="pack-detail-meta">
              {props.pack.publisher} · v{props.pack.version} · {props.pack.assetCount}{" "}
              sounds
            </p>
          </div>
        </div>
        ...
        <Show when={failed()}>
          <p class="pack-detail-error" role="alert">
            ...
            <span>“{props.pack.name}” is unavailable. Other packs still work.</span>
            <button
              type="button"
  ...
  return (
    <div class="pack-detail">
      <div class="pack-detail-heading">
        ...
        <p class="pack-detail-meta">Every pack in the library, searchable together.</p>
      </div>
...
function Results(props: {
  ...
  const { browser } = props;
  return (
    <Show when={!browser.assetsLoading()} fallback={<TapeLoader label="Loading pack" />}>
      <Show
        when={browser.results().length > 0}
        fallback={
          <p class="library-empty">No sounds match. Clear a filter to see more.</p>
        }
      >
          ...
          <For each={browser.results()}>
            {(asset) => (
              <AssetRow
                ...
                onInsert={props.onInsert ? () => props.onInsert?.(asset) : undefined}
              />
            )}
          </For>
        </ul>
        ...
        <For each={props.browser.packErrors()}>
          {(error) => {
            const name = createMemo(
              () =>
                props.browser.packs().find((pack) => pack.slug === error.packSlug)
                  ?.name ?? error.packSlug,
            );
            return (
              <p class="library-pack-error">
                <HiSolidExclamationTriangle size={14} />
                <span>
                  “{name()}” is unavailable. {LOAD_REASON_LABELS[error.reason] ?? ""}{" "}
                  Other packs still work.
                </span>
              </p>
            );
          }}
