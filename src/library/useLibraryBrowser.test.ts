...
import { type LibraryBrowserControls, useLibraryBrowser } from "./useLibraryBrowser";
...
describe("audition analytics (PRD OPS-02 / LOOP-013)", () => {
  ...
  it("emits asset_load_failed exactly once when a preview cannot load", async () => {
    ...
    await withBrowser({ analytics: a, previewEngine: engine }, async (browser) => {
      await browser.open();
      await browser.selectPack(null);
      const asset = browser.assets()[0];
...
describe("the panel tree (LOOP-013 layout)", () => {
  ...
  it("has one node per pack the project has, in index order", async () => {
    ...
    await withBrowser({ addedPackIds: [second.id, first.id] }, async (browser) => {
      await browser.open();
      expect(browser.tree().map((pack) => pack.slug)).toEqual([first.slug, second.slug]);
    });
  });
  ...
  it("loads a pack's manifest only when its node is expanded", async () => {
    ...
    await withBrowser(
      ...
      async (browser) => {
        ...
        expect(calls.filter((path) => path.includes(second.slug)).length).toBe(1);
