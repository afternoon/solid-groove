...
export function useLibraryBrowser(
  ...
  const [indexError, setIndexError] = createSignal<LibraryLoadReason | null>(null);
  const [selectedPackSlug, setSelectedPackSlug] = createSignal<string | null>(null);
  ...
  const [assetErrors, setAssetErrors] = createSignal<ReadonlyMap<string, string>>(
    new Map(),
  );
  ...
  function recordPackError(error: PackLoadError): void {
    setPackErrors((prev) => {
      if (prev.some((existing) => existing.packId === error.packId)) return prev;
      return [
        ...prev,
        {
          packId: error.packId,
  ...
  function toggleValue(key: "genres" | "roles" | "characters", value: string): void {
    setFilter((prev) => {
      const have = prev[key];
  ...
  async function retryPack(pack: LibraryPackSummary): Promise<void> {
    setPackErrors((prev) => prev.filter((error) => error.packSlug !== pack.slug));
    setAssetsLoading(true);
    try {
      await loadPack(pack);
    } finally {
