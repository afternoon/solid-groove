...
export function arrangementChunkPath(projectId: string, trackId: string): string {
...
export function encodeProjectMetadata(metadata: ProjectMetadata): StoredDocument {
...
export function encodeSong(
  ...
  const byTrack = new Map<string, { placements: JsonValue[]; automation: JsonValue[] }>();
...
export function encodeProject(project: Project): ProjectDocuments {
  ...
  const { song, arrangement } = encodeSong(id, project.song, revision, modifiedAt);
  return {
    metadata: encodeProjectMetadata(project.metadata),
    song,
    clips: project.clips.map((clip) => encodeClip(id, clip, revision, modifiedAt)),
    arrangement,
  };
}
 ...
 * of a valid project.
 */
export function decodeProject(documents: RawProjectDocuments): DecodeResult<Project> {
  const issues: PersistenceIssue[] = [];
  const metadata = asObject(documents.metadata);
  if (!metadata) {
    return {
      ok: false,
      issues: [issue("missing_document", ["metadata"], "Project metadata is missing")],
    };
  }
  ...
  documents.clips.forEach((raw, index) => {
    ...
    if (!clip) {
      issues.push(issue("invalid_shape", path, "Clip document is not an object"));
      return;
    }
...
const ENVELOPE_FIELDS = ["projectId", "schemaVersion", "revision", "updatedAt"] as const;
