...
export function describeProjectRepositoryContract(
  ...
  describe(`${label}: ProjectRepository contract`, () => {
    ...
    async function store(project: Project): Promise<Project> {
      ...
      if (!created.ok) {
        throw new Error(`createProject failed: ${created.reason} — ${created.message}`);
      }
      return project;
    }
    ...
    it("round trips a project whose arrangement overflows into chunks", async () => {
      ...
      expect(loaded.ok).toBe(true);
      if (!loaded.ok) return;
      expect(loaded.value.song.placements).toHaveLength(project.song.placements.length);
      expect(stringifyProject(loaded.value)).toBe(stringifyProject(project));
    });
    ...
    it("lists project metadata for one owner without loading song or clips", async () => {
      ...
      const theirs = await harness.repositoryFor(CONTRACT_OTHER_OWNER).createProject(
        createSliceFixtureProject({
    ...
    it("rejects a write made against a stale revision without changing state", async () => {
      ...
      expect(serializeClip(loaded.value.clips[0])).toEqual(serializeClip(newer));
      expect(loaded.value.metadata.revision).toBe(project.metadata.revision + 1);
    });
    ...
    it("reports not found when writing to a project that does not exist", async () => {
      ...
      const result = await repository.saveClip(project.metadata.id, project.clips[0], 0);
    ...
    it("exposes the pack dependency list on the metadata tier alone", async () => {
      ...
      expect(listed[0].packDependencies).toEqual(project.metadata.packDependencies);
      ...
      expect(metadata.value.packDependencies).toEqual(project.metadata.packDependencies);
    });
    ...
    it("recomputes the dependency list from the song it is saving", async () => {
      ...
      const declaredMidway = await repository.loadProjectMetadata(project.metadata.id);
      ...
      expect(metadata.value.packDependencies.map((entry) => entry.packId)).not.toContain(
        loopPackId,
      );
    ...
    it("persists an added-but-unused pack on the shelf across a reload (LIB-08)", async () => {
      ...
      expect(loaded.value.metadata.packDependencies).not.toContainEqual(shelfOnly);
    ...
    it("notifies a watcher of metadata changes and stops after unsubscribe", async () => {
      const project = await store(createSliceFixtureProject());
      const seen: number[] = [];
      const unsubscribe = repository.watchProject(project.metadata.id, (event) => {
