...
describe("song document budget", () => {
  it("would exceed the budget if the reference arrangement stayed inline", () => {
    ...
    const structureOnly = estimateDocumentBytes(inline.song.path, inline.song.data);
    ...
    expect(structureOnly + wholeArrangement).toBeGreaterThan(SONG_DOCUMENT_BUDGET_BYTES);
  });
  ...
  it("splits the 50-track ten-minute reference project into per-track chunks", () => {
    ...
    expect(encoded.arrangement).toHaveLength(referenceProject.song.tracks.length);
  ...
  it("keeps every document of the reference project inside its budget", () => {
    ...
    const songBytes = estimateDocumentBytes(encoded.song.path, encoded.song.data);
  ...
  it("round trips the chunked reference project without losing arrangement state", () => {
    ...
    expect(stringifyProject(decoded.value)).toBe(stringifyProject(referenceProject));
  });
