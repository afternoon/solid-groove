...
export interface ArrangementProjection {
  ...
  readonly placementsByTrack: ReadonlyMap<TrackId, readonly ArrangementPlacement[]>;
  readonly sections: readonly ArrangementSection[];
  readonly automation: readonly ArrangementAutomationLane[];
  readonly automationByTrack: ReadonlyMap<TrackId, readonly ArrangementAutomationLane[]>;
}
...
function clipPreviewShape(clip: Project["clips"][number]): ArrangementClipPreview {
...
export function buildArrangementProjection(
  ...
  for (const [rowIndex, track] of orderedTracks.entries()) {
    ...
    rows.push(
      buildRow(track, rowIndex, rowTop, rowHeight, previous?.rowsById.get(track.id)),
    );
    rowTop += rowHeight;
  }
  ...
  const placements = [...project.song.placements]
    ...
    .map((placement) => {
      const built = buildPlacement(placement, previousPlacements?.get(placement.id));
  ...
  const totalTicks = toTicks(
    Math.max(0, ...placements.map((p) => p.endTicks), ...sections.map((s) => s.endTicks)),
  );
