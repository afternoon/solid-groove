...
function sortDevices(devices: readonly Device[]): Device[] {
  return [...devices].sort((a, b) => a.order - b.order || compareStrings(a.id, b.id));
}
...
function buildAudioReturn(
  ...
  const fingerprint = fingerprintOf(returnFullShape(bus.order, devices, bus.mixer));
  if (
    previous &&
    previous.fingerprint === fingerprint &&
    previous.topologyFingerprint === topologyFingerprint
...
function buildAudioPlacement(
  ...
  return rememberSource({ id: placement.id, ...shape, fingerprint }, [placement]);
}
...
export function buildAudioProjection(
  ...
  const automation = [...project.song.automation]
    .sort((a, b) => compareStrings(a.id, b.id))
    .map((lane) => buildAudioAutomation(lane, previousAutomation?.get(lane.id)));
