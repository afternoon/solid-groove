  ...
  const register: RegisterSummary | null =
    pitches.length === 0
      ...
      : {
          ...
          meanPitch: Math.round(pitches.reduce((sum, p) => sum + p, 0) / pitches.length),
...
function analyzeSong(project: Project, tracks: readonly TrackAnalysis[]): SongAnalysis {
...
function collectMissing(
  ...
  if (
    ...
    missing.push({
      ...
      detail: "Every note triggers a drum pad, so there is no pitched register.",
    });
  }
  return missing;
}
...
function deriveSuggestions(
  ...
  const repetitiveTrack = tracks.find(
    (track) =>
      track.repetition.placementCount >= 4 && track.repetition.repetitionRatio >= 0.75,
  );
  if (repetitiveTrack) {
    suggestions.push({
      id: "add_variation",
...
export function buildProjectAnalysis(
  ...
  const selection = options.selection
    ? (buildAssistantContext(project, options.selection).selection?.description ?? null)
    : null;

  const recentActions = (options.recentActions ?? []).slice(0, MAX_RECENT_ACTIONS);
