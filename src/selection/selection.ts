 ...
 */
export function setFocus(state: SelectionState, scope: SelectionScope): SelectionState {
  const key = scopeKey(scope);
  const existing = findByKey(state.scopes, key);
  if (!existing) {
...
export function buildSelectionEntityIndex(project: Project): SelectionEntityIndex {
  ...
  for (const lane of project.song.automation) {
    automationPointTicks.set(lane.id, new Set(lane.points.map((point) => point.tick)));
  }
...
function scopeIsLive(
  ...
  switch (scope.kind) {
    ...
    case "barRange": {
      ...
      const survivingTrackIds = scope.trackIds.filter((id) => index.trackIds.has(id));
