...
describe("matchesChord", () => {
	it("matches the character the layout produced, not a physical key code", () => {
		...
		expect(
			matchesChord(chord, keyEvent("?", { shiftKey: true }), "other"),
		).toBe(true);
	...
	it("matches letters case-insensitively but keeps Shift significant", () => {
		...
		expect(
			matchesChord(chord, keyEvent("E", { shiftKey: true }), "other"),
		).toBe(false);
	});
	...
	it("separates Space from Shift+Space", () => {
		...
		expect(matchesChord(play, keyEvent(" ", { shiftKey: true }), "other")).toBe(
			false,
		);
		expect(
			matchesChord(contin, keyEvent(" ", { shiftKey: true }), "other"),
		).toBe(true);
	});

	it("does not let the wrong primary modifier through", () => {
		const chord = parseChord("Mod+Z", "mac");
		expect(matchesChord(chord, keyEvent("z", { metaKey: true }), "mac")).toBe(
			true,
		);
		expect(matchesChord(chord, keyEvent("z", { ctrlKey: true }), "mac")).toBe(
			false,
		);
		const other = parseChord("Mod+Z", "other");
		expect(matchesChord(other, keyEvent("z", { metaKey: true }), "other")).toBe(
			false,
		);
	});
	...
	it("requires Alt to match exactly", () => {
		...
		expect(
			matchesChord(
				chord,
				keyEvent("d", { ctrlKey: true, altKey: true }),
				"other",
			),
		).toBe(false);
	});
...
describe("labels", () => {
	...
	it("names keys that are not printable characters", () => {
		...
		expect(chordLabel(parseChord("Shift+Space", "mac"), "mac")).toBe(
			"Shift+Space",
		);
	});
});
